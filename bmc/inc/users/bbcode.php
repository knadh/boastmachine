...
// bbCode parsing engine adopted from phpBB2 (http://phpbb.com)


//error_reporting(E_ALL ^ E_NOTICE);

function bmc_bbCode($text=null) {
	...
	// Unique ID for this text..
	$uid = md5(mt_rand());
	$uid = substr($uid, 0, 10);


	return bbencode_second_pass(bbencode_first_pass($text,$uid ),$uid);
...
{
	global $template;
	$tpl = fread(fopen(CFG_ROOT."/inc/users/bbcode.inc", "r"), filesize(CFG_ROOT."/inc/users/bbcode.inc"));
...
function prepare_bbcode_template($bbcode_tpl)
{
	...
	$bbcode_tpl['font_open'] = str_replace('{FONT}', '\\1', $bbcode_tpl['font_open']);	// boastMachine 3.1

	$bbcode_tpl['size_open'] = str_replace('{SIZE}', '\\1', $bbcode_tpl['size_open']);

	$bbcode_tpl['quote_open'] = str_replace('{L_QUOTE}', "QUOTE:", $bbcode_tpl['quote_open']);

	$bbcode_tpl['code_open'] = str_replace('{L_CODE}', "CODE:", $bbcode_tpl['code_open']);
...
function bbencode_second_pass($text, $uid)
{
	...
	// fonts (boastMachine 3.1)
	$text = preg_replace("/\[font=([a-z\s]+):$uid\]/si", $bbcode_tpl['font_open'], $text);
	...
	// [s] and [/s] for striking text. (boastMachine 3.1)
	...
	// matches a [url]xxxx://www.site.com[/url] code..
	$patterns[] = "#\[url\]([\w]+?://[^ \"\n\r\t<]*?)\[/url\]#is";
	$replacements[] = $bbcode_tpl['url1'];

	// [url]www.site.com[/url] code.. (no xxxx:// prefix).
	$patterns[] = "#\[url\]((www|ftp)\.[^ \"\n\r\t<]*?)\[/url\]#is";
	$replacements[] = $bbcode_tpl['url2'];

	// [url=xxxx://www.site.com]Site[/url] code..
	$patterns[] = "#\[url=([\w]+?://[^ \"\n\r\t<]*?)\](.*?)\[/url\]#is";
	$replacements[] = $bbcode_tpl['url3'];

	// [url=www.site.com]Site[/url] code.. (no xxxx:// prefix).
	$patterns[] = "#\[url=((www|ftp)\.[^ \"\n\r\t<]*?)\](.*?)\[/url\]#is";
...
function bbencode_first_pass($text, $uid)
{
	...
	// [font] and [/font] for setting font (boastMachine 3.1)
	$text = preg_replace("#\[font=([a-z\s]+)\](.*?)\[/font\]#si", "[font=\\1:$uid]\\2[/font:$uid]", $text);
	...
	// [bs] and [/s] for striking text.(boastMachine 3.1)
