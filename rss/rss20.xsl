...
<xsl:template match="/">
  ...
  <body>
    <xsl:attribute name="style">font-family: 'Trebuchet MS'; font-size: 10pt; background-color: #fafafa;</xsl:attribute>
...
Feed Title: <b><xsl:value-of select="title"/></b><br />
Updated on: <xsl:value-of select="pubDate"/><br />
<xsl:choose>
<xsl:when test="count(item) = 1">
There is only one post shown in this feed.
</xsl:when>
<xsl:otherwise>
...
This XML feed has been formatted using XLS styling. View the source code for XML
</i>
</p>
